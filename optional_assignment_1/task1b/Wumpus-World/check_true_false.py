...
def check_true_false(knowledge_base, statement):
    ...
    model = {}
    getConstants(model, knowledge_base)

    symbols = list(symbolsList)

    truthValues = tt_check_all(knowledge_base, statement, symbols, model)

    print '\nFinished check. Check result.txt for output'

    output = open('result.txt', 'w')
    if len(truthValues) == 0:
    	output.write("both true and false\n")
    elif ((True in truthValues) and (False in truthValues)):
    	output.write("possibly true, possibly false\n")
    elif all(truthValues):
